 ...
 */
public class VideoPlayerNew extends BasisActivity {
...
//	}
	...
	private void playVideo() {// finish是否从头开始
		...
		if (SettingConfig.Instance().isHighSpeed()) {
			append = "http://staticvip."+com.iyuba.core.util.Constant.IYBHttpHead+"/video/voa/"
					+ newsArrayList.get(listPosition).id + ".mp4";
		} else {
			append = WebConstant.HTTP_STATIC+com.iyuba.core.util.Constant.IYBHttpHead+"/video/voa/"
					+ newsArrayList.get(listPosition).id + ".mp4";
		}
		url = append;
